...
# Ask user for transaction ID filter
filter_txn = input("Enter Transaction ID to filter (leave blank for all): ").strip()

filtered_txns = transactions
if filter_txn:
    if filter_txn in transactions:
        filtered_txns = {filter_txn: transactions[filter_txn]}
    else:
        print(f"❌ Transaction ID {filter_txn} not found in log")
        exit()
